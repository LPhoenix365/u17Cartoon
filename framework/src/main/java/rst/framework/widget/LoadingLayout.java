...
//        mCenterLoadAnim.setController(controller);
    ...
    public void showNoDataError() {
        if (nodataLayout == null) {
            ...
            mErrorBtn = nodataLayout.findViewById(R.id.error_btn);
            if (mRefreshClickListener != null) {
                if (mErrorBtn != null) {
                    mErrorBtn.setOnClickListener(mRefreshClickListener);
                }
            }
...
/**
     ...
     * 隐藏刷新按钮
     */
    public void hideErrorLayoutButton() {
        if (mErrorBtn != null) {
    ...
    public void onTaskFail(NetwrokTaskError error, String serverTag) {
        ...
        switch (error.errorCode) {
            case NetwrokTaskError.TaskCodeList.BusinessError:
                if (TextUtils.isEmpty(error.errorString)) {
                    updateErrorText("数据异常");
                } else {
                    updateErrorText(error.errorString);
                }
                break;
            case NetwrokTaskError.TaskCodeList.NetworkError:
                updateErrorText("网络连接异常");
                break;
            case NetwrokTaskError.TaskCodeList.ServerError:
                updateErrorText("网络连接异常");
                break;
            case NetwrokTaskError.TaskCodeList.TimeoutError:
