package rst.framework.network;

import android.text.TextUtils;

import com.google.gson.Gson;
import com.google.gson.JsonSyntaxException;

import rst.framework.FrameWorkConfig;
import rst.framework.FrameworkConstants;
import rst.framework.model.response.BaseResponse;
import rst.framework.utils.LogUtil;

/**
 * Created by iceman on 15/5/14.
 * 业务数据解析时使用的结果封装
 * 非网络错误会在这里标记出来
 */
public class BusinessResult {
    public boolean isSuccess = true;
    public String errorString = "";

    public BusinessResult() {

    }
    public BusinessResult(boolean success) {
        isSuccess=success;
    }


    public BusinessResult(String s) {
        if (!TextUtils.isEmpty(s)) {
            try {
                BaseResponse response = new Gson().fromJson(s, BaseResponse.class);
                if (response == null) {
                    isSuccess = false;
                    return;
                }
                if (response.code == FrameworkConstants.CODE_BUSINESS_SUCCESS || response.code == FrameworkConstants.CODE_CMS_SUCCESS) {
                    errorString = "";
                } else {
                    LogUtil.d("数据异常!response code:" + response.code);
                    isSuccess = false;
                    errorString = response.message;
                    if (response.code == FrameworkConstants.CODE_BUSINESS_FAIL && !TextUtils.isEmpty(response.message)) {
                        if (response.message.equals("请先在<个人信息>中绑定身份证号")) {
                            if (FrameWorkConfig.frameworkSupport != null) {
                                FrameWorkConfig.frameworkSupport.onCardInValid();
                            }
                        }
                    }
                }
            } catch (JsonSyntaxException e) {
                e.printStackTrace();
            }
        }
    }

    public BusinessResult(BaseResponse response) {
        if (response == null) {
            isSuccess = false;
            return;
        }
        if ((response.code == FrameworkConstants.CODE_BUSINESS_SUCCESS) || (response.code ==FrameworkConstants.CODE_CMS_SUCCESS)) {
            errorString = "";
        } else {
            LogUtil.d("数据异常!response code:" + response.code);
            isSuccess = false;
            errorString = response.message;
            if (response.code == FrameworkConstants.CODE_BUSINESS_FAIL && !TextUtils.isEmpty(response.message)) {
                if (response.message.equals("请先在<个人信息>中绑定身份证号")) {
                    if (FrameWorkConfig.frameworkSupport != null) {
                        FrameWorkConfig.frameworkSupport.onCardInValid();
                    }
                }
            }
        }
    }
}
