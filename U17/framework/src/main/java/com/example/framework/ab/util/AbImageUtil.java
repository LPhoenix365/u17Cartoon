...
import com.paic.mhis.siapp.ab.util.dct.FDCT;
import com.paic.mhis.siapp.utils.CommonUtils;
import com.paic.mhis.siapp.utils.ImageCompress;
...
public class AbImageUtil {
     ...
     *
     * 修改 @date:2016 7-28 10:25 修改图片压缩方法(武义工伤报案) by fengzhengwen
     */
    public static String getScaleBitmapPath(Context context, String path, int desiredWidth, int desiredHeight, int quality) {
        String picPath = "";
        if (!CommonUtils.isEmpty(creatTempFile(context))) {//判断临时图片文件夹是否存在
            picPath = creatTempFile(context);
            ImageCompress.compressFromUri(path, desiredWidth, desiredHeight, picPath, quality);
        }
        return picPath;
    }
    ...
    public static String creatTempFile(Context context) {
        String path = "";
        if (Environment.getExternalStorageState() != null && Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {//判断SD卡是否存在
            String packagePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Android/data/" + context.getPackageName() + "/files/photo/";//得到图片绝对路径
            File packageFile = new File(packagePath);
            if (!packageFile.exists()) { //路径不存在就创建
                packageFile.mkdirs();
            }
            path = packagePath + System.currentTimeMillis() + ".jpg"; //得到图片的存储路径

        } else {
            ToastUtil.showToast(context, "SD卡异常,请检查SD卡是否插入");
        }
        return path;
    }
...
//      return compressBmpFromBmp(bitmap);//原来的方法调用了这个方法企图进行二次压缩
    ...
    public static String getHashCode(Bitmap bitmap) {
        ...
        AbLogUtil.i("th", "将图片缩小到8x8的尺寸:" + width + "*" + height);
    ...
    public static String getDCTHashCode(Bitmap bitmap) {
        ...
        int width = temp.getWidth();
        int height = temp.getHeight();
        AbLogUtil.i("th", "将图片缩小到32x32的尺寸:" + width + "*" + height);
