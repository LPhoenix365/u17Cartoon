...
public class AuthSSLProtocolSocketFactory implements LayeredSocketFactory {
	 ...
	 */
	public AuthSSLProtocolSocketFactory(final int keystoreId,
			final String keystorePassword, final int truststoreId,
			final String truststorePassword,final Context context) {
		super();
	...
	private static KeyStore createKeyStore(final int id, final String password,final Context context)
	...
	private SSLContext createSSLContext() {
		try {
			...
			if (this.keystoreId != -1) {
				KeyStore keystore = createKeyStore(this.keystoreId,
						this.keystorePassword,context);
				if (AbLogUtil.D) {
			...
			if (this.truststoreId != -1) {
				KeyStore keystore = createKeyStore(this.truststoreId,
						this.truststorePassword,context);
				if (AbLogUtil.D) {
					Enumeration aliases = keystore.aliases();
					while (aliases.hasMoreElements()) {
						String alias = (String) aliases.nextElement();
