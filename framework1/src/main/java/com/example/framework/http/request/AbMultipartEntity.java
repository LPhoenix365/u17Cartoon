...
//        boundaryEnd = ("--" + boundary + "--\r\n").getBytes();
    ...
    public AbMultipartEntity(AbHttpResponseListener responseListener,String resultBoundary) {
        final StringBuilder buf = new StringBuilder();
        final Random rand = new Random();
        for (int i = 0; i < 30; i++) {
            buf.append(MULTIPART_CHARS[rand.nextInt(MULTIPART_CHARS.length)]);
    ...
    private byte[] createContentType(String type) {
        ...
        AbLogUtil.d("createContentType:"+result);
        try {
    ...
    private byte[] createContentDisposition(final String key) {
        AbLogUtil.d("Content-Disposition: form-data; name=\"" + key + "\"\r\n");
        try {
    ...
    private byte[] createContentDisposition(final String key, final String fileName) {
        AbLogUtil.d("createContentDisposition Content-Disposition: form-data; name=\"" + key + "\"; filename=\"" + fileName + "\"\r\n");
        try {
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     * @see HttpEntity#writeTo(OutputStream)
     */
    @Override
...
//                headerStream.write(TRANSFER_ENCODING_BINARY);
                ...
                AbLogUtil.d("createHeader out:"+new String(headerStream.toByteArray(), "utf-8"));
                AbLogUtil.d("createHeader createContentDisposition"+createContentDisposition(key, filename).toString());
            } catch (Exception e) {
                e.printStackTrace();
